...
const sources = ['Salary', 'Freelance', 'Business', 'Investments', 'Rental', 'Bonus', 'Other'];

const IncomeModal = ({ isOpen, mode, form, onChange, onSubmit, onClose, saving }) => {
  if (!isOpen) {
    return null;
  }
    ...
    >
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 id="income-modal-title" className="text-2xl font-bold text-slate-100">
              {mode === 'edit' ? 'Edit income' : 'Add income'}
            </h2>
            <p className="mt-1 text-sm text-slate-400">
              Track earned money and keep your dashboard totals in sync.
            </p>
          </div>
        ...
        <form className="mt-6 grid gap-4 md:grid-cols-2" onSubmit={onSubmit}>
          ...
          <label className="grid gap-2 md:col-span-2">
            ...
            <textarea
              ...
              value={form.notes}
              onChange={onChange}
              placeholder="Optional note about the income"
              rows="4"
            />
          </label>
          ...
          <div className="flex flex-wrap gap-3 md:col-span-2 md:justify-end">
            ...
            >
              {saving ? 'Saving...' : mode === 'edit' ? 'Update income' : 'Create income'}
            </button>
          </div>
        </form>
      </motion.div>
